...
summaryPlpAnalyses <- function(analysesLocations){ 
  
  analysesLocations_list <- list.dirs(path = analysesLocations, recursive = F, full.names = F)
  allPerformances <- c()
  
  for (analysesLocation_dir in analysesLocations_list) {
   
    analysesLocation <- file.path(analysesLocations, analysesLocation_dir)
    ...
    allPerformances <- rbind(allPerformances, allPerformance)
  }
...
getPerformance <- function(analysisLocation){
  ...
  if(type == 'csv'){
    ...
    name <- strsplit(analysisLocation, "/")
    
    res$model$trainDetails$cdmDatabaseSchema <- name[[1]][7]
    result <- getSummaryFromObject(result = res, analysisId = analysisId)
