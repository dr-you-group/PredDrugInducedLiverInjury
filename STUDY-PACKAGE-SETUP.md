...
The setup script below is used to install the **PredDrugInducedLiverInjury** package. You will need to modify this setup script as follows:
                                                                                              
- Set the `projectRootFolder` variable to the directory specific to your environment. In this example we are using `C:/PredDrugInducedLiverInjury`. This root folder will serve a few purposes:
...
projectFolder <- "C:/PredDrugInducedLiverInjury"
setwd(projectFolder)
                                                                                                 
# Download the lock file:
download.file("https://raw.githubusercontent.com/ohdsi-studies/PredDrugInducedLiverInjury/master/renv.lock", "renv.lock")
...
You can safely continue by pressing 'y' after this prompt since the renv.lock file is downloaded from the **PredDrugInducedLiverInjury** GitHub code repository. Once the installation is complete, you may need to restart R (if you are working outside of RStudio) and you should see this message:

````
Project 'C:/PredDrugInducedLiverInjury' loaded. [renv 0.x.y]
````
